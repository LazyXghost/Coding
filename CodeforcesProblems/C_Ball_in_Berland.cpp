...
struct solution{
    ...
    solution(){
        ...
        loop(i,0,k-1,1)
        {
            ...
            cin>>x;
            brr.push_back(x);
            resizer = max(resizer, x+mapper);
        }
        graph.resize(resizer+1);
        state.resize(resizer+1,-1);
        visited.resize(resizer+1,0);

        loop(i,0,k-1,1){
            graph[arr[i]].push_back(brr[i]+mapper);
            graph[brr[i]+mapper].push_back(arr[i]);
        }
